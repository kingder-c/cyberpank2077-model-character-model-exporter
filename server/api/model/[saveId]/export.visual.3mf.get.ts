...
export default defineEventHandler(async (event) => {
  ...
  const filePath = getArtifactFile(saveId, "visual3mf");
  if (!filePath) {
    throw createError({ statusCode: 404, statusMessage: "还没有生成 3MF 彩色预览模型，请先点击生成/刷新模型。" });
  }

  setHeader(event, "Content-Type", "model/3mf");
  setHeader(event, "Content-Disposition", `attachment; filename="cyberpunk-v-${saveId}-visual.3mf"`);
